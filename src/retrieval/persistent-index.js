...
const { overlapScore, exampleSearchText, loreSearchText } = require('./lexical');
...
class PersistentEmbeddingIndex {
  constructor({ filePath, embedder, batchSize = 64, modelKey = null } = {}) {
    ...
    this.records = parsed.modelKey === this.modelKey && parsed.records && typeof parsed.records === 'object'
      ? parsed.records
      : {};
  }

  save() {
    writeJsonAtomic(this.filePath, { version: 1, modelKey: this.modelKey, records: this.records });
  }
  ...
  async sync(items, { domain = 'default', textFn = (item) => String(item?.content ?? ''), idFn = defaultId } = {}) {
    ...
    for (let index = 0; index < list.length; index += 1) {
      ...
      const key = `${domain}:${idFn(item, index)}`;
      const text = textFn(item);
      const hash = fingerprint(text);
      expected.add(key);
      if (!this.records[key] || this.records[key].fingerprint !== hash) pending.push({ key, text, hash });
    }

    for (let offset = 0; offset < pending.length; offset += this.batchSize) {
      const batch = pending.slice(offset, offset + this.batchSize);
      const vectors = await this.embedder.embed(batch.map((entry) => entry.text));
    ...
    for (const key of Object.keys(this.records)) {
      if (key.startsWith(`${domain}:`) && !expected.has(key)) {
  ...
  async retrieve(items, query, k = 6, {
    domain = 'default',
    textFn = (item) => String(item?.content ?? ''),
    idFn = defaultId,
    lexicalWeight = 0.35,
    semanticWeight = 0.65,
  } = {}) {
    ...
    return list
      .map((item, index) => {
        const key = `${domain}:${idFn(item, index)}`;
        const lexical = overlapScore(query, textFn(item));
        const semantic = Math.max(0, cosineSimilarity(queryVector, this.records[key]?.vector));
        return {
          item,
          index,
          lexical,
          semantic,
          score: lexicalWeight * lexical + semanticWeight * semantic,
        };
      })
...
function createIndexedVoiceRetriever({ index, ...options } = {}) {
  if (!index) throw new TypeError('index is required');
  return {
    retrieve(items, query, k) {
      return index.retrieve(items, query, k, { domain: 'voice', textFn: exampleSearchText, ...options });
    },
  };
}

function createIndexedLoreRetriever({ index, ...options } = {}) {
  if (!index) throw new TypeError('index is required');
  return {
    retrieve(items, query, k) {
      return index.retrieve(items, query, k, { domain: 'lore', textFn: loreSearchText, ...options });
    },
  };
}
