...
function formatExample(example) {
  ...
  if (Array.isArray(example.context) && example.context.length) {
    lines.push('Context:');
    for (const entry of example.context.slice(-4)) lines.push(`${clip(entry.authorName, 60)}: ${clip(entry.content, 180)}`);
  }
  if (example.repliedTo) lines.push(`Replying to ${clip(example.repliedTo.authorName, 60)}: ${clip(example.repliedTo.content, 220)}`);
  ...
  } else lines.push(`Character: ${clip(example.content, 320)}`);
  return lines.join('\n');
}
...
function buildSystemPrompt(pack, context = {}) {
  ...
  sections.push('## Stable identity boundary\nStatements made by conversation participants describe those participants, not you. Never absorb a speaker\'s name, relationship, biography, preferences or history as your own identity unless the canonical Character Pack says so.');
  ...
  if (voice.length) sections.push(`## Voice reference\nUse these as behavioral/voice examples, not canonical world facts.\n\n${voice.map(formatExample).join('\n\n')}`);
  if (context.retrievedLore?.length) sections.push(`## Retrieved lore\nCanonical/reference facts relevant to this turn. Prefer these over guessing.\n${context.retrievedLore.map(formatLore).join('\n')}`);
  if (context.relationshipNote) sections.push(`## Relationship with current speaker\n${clip(context.relationshipNote, 1200)}`);
  if (context.memories?.length) sections.push(`## User-specific durable memories\n${context.memories.map((entry) => `- ${clip(entry.fact ?? entry, 400)}`).join('\n')}`);
  if (context.episodes?.length) sections.push(`## Past shared episodes with current speaker\nThese are compact recollections of prior interactions, not canonical lore. Use only when relevant.\n${context.episodes.map((entry) => `- ${clip(entry.summary ?? entry, 500)}`).join('\n')}`);
...
function buildUserPrompt({ message, speaker, history = [] }) {
  const recent = history.slice(-12).map((entry) => ({ speakerId: entry.speakerId ?? null, speakerName: clip(entry.speakerName || '', 80), content: clip(entry.content || '', 800) }));
